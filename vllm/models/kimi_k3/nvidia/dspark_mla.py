...
class K3DSparkDecoderLayer(nn.Module):
    @staticmethod
    def get_attention_window(config, layer_idx: int) -> int | None:
        return None
    ...
    def __init__(
        ...
        *,
        ...
        self.self_attn = MultiHeadLatentAttention(
            ...
            draft_kv_window=self.get_attention_window(config, layer_idx),
        )
...
class K3DSparkForCausalLM(nn.Module):
    ...
    def supports_local_draft_argmax(self) -> bool:
        ...
        if self.logits_processor.soft_cap is not None:
            return False
        return self.logits_processor.scale == 1.0
    ...
    def get_b12x_preparation_units(self, layer, workload: B12xWorkload):
        ...
        if self._argmax_runtime is None:
            self._argmax_runtime = PCIeVocabParallelArgmax.from_exchange_group(
                exchange_group=get_tp_group().cpu_group,
                device=self.lm_head.weight.device,
                local_vocab_size=self.lm_head.num_embeddings_per_partition,
                max_batch_size=self._argmax_capacity,
            )
            self._argmax_output = torch.empty(
                self._argmax_capacity,
                dtype=torch.int64,
                device=self.lm_head.weight.device,
            )
        runtime = self._argmax_runtime
        if self._argmax_plan is None:
            query = _owner_preparation.query_from_runtime(runtime)
            self._argmax_plan = _owner_preparation.plan(query, runtime=runtime)

        def prepare(state):
            base = torch.full(
                (runtime.max_batch_size, runtime.local_vocab_size),
                0.5,
                dtype=torch.bfloat16,
                device=runtime.device,
            )
            bias = torch.full_like(base, 0.25)
            return _owner_preparation.prepared_call(
                state, base=base, bias=bias, out=self._argmax_output
            )

        name = "kimi_dspark.vocab_argmax"
        return (
            B12xPreparationUnit(
                name="DSPARK_VOCAB_ARGMAX",
                key=(
                    runtime.world_size,
                    runtime.local_vocab_size,
                    self._argmax_capacity,
                ),
                requests=(
                    self._argmax_plan.request(
                        name=name,
                        prepare_call=prepare,
                        collective=_owner_preparation.collective(runtime, key=name),
                    ),
                ),
                stage="weights",
            ),
    ...
    def sample_local_draft_logits(
        ...
        if (
            self._argmax_enabled
            and self.lm_head.tp_size in (8, 12, 16)
            and base_logits.dtype == markov_bias.dtype == torch.bfloat16
            and 0 < batch <= self._argmax_capacity
        ):
            if self._argmax_plan is None:
            ...
            return self._argmax_runtime.fused_add_argmax(
                base_logits,
                markov_bias,
                out=self._argmax_output[:batch],
                plan=self._argmax_plan,
            )
