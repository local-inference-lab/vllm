...
class KimiMLP(nn.Module):
    ...
    def __init__(
        ...
        if pad_intermediate or (
            quant_config is not None and quant_config.get_name() == "qsrt_k2"
        ):
...
class KimiMoE(nn.Module):
    def __init__(
        ...
        self._aligned_decode_projections = (
            source_aligned_qsrt and envs.VLLM_KIMI_ALIGNED_DECODE_PROJECTIONS
        )
        self.register_buffer("_paired_decode_weight", None, persistent=False)
        if source_aligned_qsrt:
            from vllm.model_executor.layers.quantization.utils.qsrt_k2 import (
                plan_qsrt_k2_extent,
            )

            # Reserve the widest rank's activation capacity, not padded weights.
            self.padded_moe_intermediate_size = self.tp_size * max(
                plan_qsrt_k2_extent(layer_idx, self.tp_size, rank).intermediate_size
