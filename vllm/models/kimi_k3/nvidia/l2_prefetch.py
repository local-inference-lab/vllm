"""L2 weight prefetch for Kimi-K3 decode on SM120.

At decode batch sizes the dense projections of a layer are device-memory
(GDDR7) streams of a few to a few tens of megabytes each (KDA
``in_proj_qkvgfab`` 22.7 MB per rank at TP8, MLA ``fused_qkv_a_proj`` 11 MB,
``o_proj`` 7.6 MB), read while the SMs are mostly waiting for them. The
routed-expert kernel, the PCIe all-reduces and the attention core leave
device memory idle for a large part of every layer. This module issues
``cp.async.bulk.prefetch.L2`` with an ``evict_last`` cache policy for the
*upcoming* dense weights on a side stream inside those windows, so the
GEMMs read them from L2 (128 MB on RTX PRO 6000 Blackwell). Measured on one
RTX PRO 6000 with the served b12x MXFP8 dense GEMM at M=4 (kernel time,
cold L2 -> prefetched): 22.7 MB 31.1 -> 21.4 us, 7.6 MB 9.5 -> 7.0 us,
11.4 MB 23.9 -> 20.2 us.

Numerics are untouched (cache hints only). The side stream is forked per
window and rejoined once at the end of the model forward, which is valid
in FULL cudagraph captures and eager runs; inside a breakable (PIECEWISE)
...
Windows per decoder layer:
  A  inside attention after the first projection: this layer's o_proj
     (and the absorbed MLA W_UK_T / W_UV)
  B  before the attention-output all-reduce: the MoE router weight and the
     next layer's first projections
  C  before the MoE all-reduce: the remainder

Environment:
  VLLM_K3_L2_PREFETCH=0              disable (default: on for SM120)
  VLLM_K3_L2_PREFETCH_MAX_TOKENS     only prefetch for batches up to this (256)
  VLLM_K3_L2_PREFETCH_BUDGET_{A,B,C,A_MLA}_MB   per-window fill budgets
  VLLM_K3_L2_PREFETCH_A_NEXT_MB      next-layer head bytes carried in window A
  VLLM_K3_L2_PREFETCH_PERSIST_MB     persisting-L2 set-aside per rank: "max",
...
``VLLM_K3_L2_PREFETCH_PERSIST_MB`` sizes that process-wide set-aside once per
device through ``cuCtxSetLimit``. This is a cache-residency policy only;
kernels and numerics are unchanged. Ported from the GLM-5.3 module of
local-inference-lab/vllm#586.
"""
...
def _int_env(name: str, default: int) -> int:
    ...
    try:
        ...
        logger.warning("[k3 l2_prefetch] ignoring %s=%r", name, raw)
        return default
...
_MAX_TOKENS = _int_env("VLLM_K3_L2_PREFETCH_MAX_TOKENS", 256)


BUDGET_A = _mb("VLLM_K3_L2_PREFETCH_BUDGET_A_MB", "20")
...
BUDGET_B = _mb("VLLM_K3_L2_PREFETCH_BUDGET_B_MB", "50")
BUDGET_C = _mb("VLLM_K3_L2_PREFETCH_BUDGET_C_MB", "15")
BUDGET_A_MLA = _mb("VLLM_K3_L2_PREFETCH_BUDGET_A_MLA_MB", "36")
# Measured neutral-to-negative on C1 (fills overlap the KDA core); off by default.
A_NEXT_BYTES = _mb("VLLM_K3_L2_PREFETCH_A_NEXT_MB", "0")


def _persisting_l2_env_request() -> str:
    """Return the process request, leaving the driver limit unchanged by default."""
    return os.getenv("VLLM_K3_L2_PREFETCH_PERSIST_MB", "0")
...
def _platform_enabled() -> bool:
    ...
    if not torch.cuda.is_available():
        return False
    raw = os.getenv("VLLM_K3_L2_PREFETCH")
    if raw is not None:
        return raw != "0"
...
def _get_launcher():
    ...
    if not _CUTE_OK:
        ...
        logger.warning("[k3 l2_prefetch] disabled: CuTe DSL not available")
        return None
    try:
        ...
        _compiled = launch
        logger.info(
            "[k3 l2_prefetch] CuTe kernel ready (grid=%d block=%d chunk=%d; "
        ...
        _compile_failed = True
        logger.warning("[k3 l2_prefetch] disabled: kernel compile failed: %s", exc)
        return None
...
def persisting_l2_request(raw: str | None, max_bytes: int) -> int:
    ...
    try:
        ...
        logger.warning(
            "[k3 l2_prefetch] ignoring VLLM_K3_L2_PREFETCH_PERSIST_MB=%r "
...
def configure_persisting_l2(device: torch.device, request: str | None = None) -> int:
    ...
    try:
        ...
        logger.info(
            "[k3 l2_prefetch] persisting L2 set-aside on cuda:%d: L2 %.0f MB, "
        ...
        logger.warning(
            "[k3 l2_prefetch] persisting L2 set-aside not applied on cuda:%d: %s",
            idx,
            exc,
        )
        applied = 0
