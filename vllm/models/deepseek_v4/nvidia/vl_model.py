...
def _make_deepseek_v4_vl_weights_mapper(
    ...
    return WeightsMapper(
        ...
        orig_to_new_substr={
            ...
            # Draft heads are not supported for the vision variant; drop
            # their weights.
            "mtp.": None,
        },
    )
...
class DeepseekV4ForConditionalGeneration(
    ...
    def get_language_model(self) -> nn.Module:
        """Expose the inner text backbone for a draft-model loader."""
        return self.language_model

    def get_mtp_target_hidden_states(self) -> torch.Tensor | None:
        """Return draft hidden states captured after vision embeddings merge."""
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        # Map HF names into this wrapper's namespace. NO sorted()
        # materialization: holding every weight at once prevents streaming on
        # memory-constrained systems. Instead the "language_model." tensors
        # stream through immediately (one contiguous run means the child's
        # load_weights finalizes exactly once at its end), while only the
        # small vision/aligner/sentinel groups are deferred to the tail.
        mapped = self.hf_to_vllm_mapper.apply(weights)
        deferred: list[tuple[str, torch.Tensor]] = []

        def _ordered() -> Iterable[tuple[str, torch.Tensor]]:
            for name, tensor in mapped:
                if name.startswith("language_model."):
                    yield name, tensor
                else:
                    deferred.append((name, tensor))
            yield from deferred

        loader = AutoWeightsLoader(self)
        loaded_params = loader.load_weights(_ordered())
