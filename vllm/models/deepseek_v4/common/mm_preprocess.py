...
class DeepseekV4VLMultiModalProcessor(
    ...
    def _apply_hf_processor_text_mm(
        ...
        """Tokenize text and process images with the model-specific processor.

        The model processor does not return ``input_ids``, while the base
        ``_apply_hf_processor_text_mm`` expects them. The model-specific image
        pipeline also does not follow the standard
        ``image_processor(images=...)`` / ``_merge_kwargs`` convention. The
        prompt contains the tokenizer's ``<｜deepseek_image｜>`` placeholder;
        sentinel blocks and alignment padding are applied later through
        ``_maybe_apply_prompt_updates`` (``is_update_applied=False``).
        """
        tokenizer = self.info.get_tokenizer()
        prompt_ids = tokenizer.encode(prompt_text, **cast(Any, tokenization_kwargs))
        return (
            prompt_ids,
    ...
    def _apply_hf_processor_mm_only(
        ...
        """Process image-only inputs with the model-specific pipeline.

        The base implementation calls ``call_hf_processor_mm_only``, which
        requires the Transformers ``image_processor(images=...)`` convention.
        This model supplies its vision inputs through a separate pipeline.
        """
        patches_list = []
        vit_grid = []
        llm_grid = []
        ...
        if images is not None:
            ...
            image_processor = DeepseekV4VLImageProcessor(config)
            for idx in range(len(images)):
                image = images.get(idx)
                if isinstance(image, np.ndarray):
                    image = Image.fromarray(image)
                patches, n_vit_h, n_vit_w, n_llm_h, n_llm_w = image_processor(image)
                types, perm = build_image_block_pad_free(n_llm_h, n_llm_w)
                patches_list.append(patches)
                vit_grid.append((n_vit_h, n_vit_w))
                llm_grid.append((n_llm_h, n_llm_w))
    ...
    def _apply_token_matches_with_placeholders(
        ...
        """Apply token replacements and add image alignment padding.

        The processor API no longer exposes this hook. Its equivalent flow is
        token matching (with a text fallback), placeholder discovery, and then
        image-specific compressor alignment padding
        (``3 - start_pos % 4`` IMAGE_PAD sentinels) before each image block.
        Keep the three-value return signature expected by the multimodal
        ...
        matched_updates = defaultdict[str, list[Sequence[Any]]](list)
        ...
        placeholders = self._find_mm_placeholders(new_token_ids, dict(matched_updates))

        pad_id = IMAGE_SENTINEL_BASE_ID + IMAGE_PAD
        ids = list(new_token_ids)
        updated: dict[str, list[PlaceholderFeaturesInfo]] = {
            modality: list(infos) for modality, infos in placeholders.items()
        }
        shift = 0
        for idx, info in enumerate(list(updated.get("image", []))):
            start = info.start_idx + shift
            compress_pad = COMPRESS_PAD_TO - 1 - start % COMPRESS_PAD_TO
            if compress_pad:
                pad_tokens = [pad_id] * compress_pad
                ids[start:start] = pad_tokens
                shift += compress_pad
            pad_is_embed = torch.zeros(compress_pad, dtype=torch.bool)
            is_embed = (
                torch.cat([pad_is_embed, info.is_embed])
                if info.is_embed is not None
                else None
            )
            updated["image"][idx] = PlaceholderFeaturesInfo(
                modality=info.modality,
                item_idx=info.item_idx,
                # ``start_idx`` is the position before padding, matching
                # ``build_image_block``; the padding itself is in ``tokens``.
                start_idx=start,
                tokens=[pad_id] * compress_pad + list(info.tokens),
                is_embed=is_embed,
            )

        return ids, match_result, updated
    ...
    def _apply_prompt_updates(
        ...
        """Apply base prompt replacements and image alignment padding."""
        new_token_ids, _, placeholders = self._apply_token_matches_with_placeholders(
            token_ids, mm_prompt_updates
        )
