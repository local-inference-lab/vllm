...
def _warm_chunk_kda_prefill(
    ...
    """Autotune the production Triton KDA prefill shape without KV pages."""
...
def kimi_k3_triton_prefill_warmup(worker: Worker) -> bool:
    """Autotune target KDA prefill before the real KV cache is allocated."""
    ...
    return _warm_chunk_kda_prefill(layer, worker.model_config.dtype)
...
def kimi_k3_triton_warmup(worker: Worker) -> None:
    ...
    kimi_k3_triton_prefill_warmup(worker)
