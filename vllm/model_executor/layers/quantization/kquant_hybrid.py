    ...
    *,
    ...
    for metadata_group in metadata_groups:
        ...
        if not valid_metadata:
            break
    if not valid_metadata:
        result = False if metadata_groups else None
    elif not phase_counts:
        result = None
    else:
        result = all(
...
class KQuantHybridMoEMethod(FusedMoEMethodBase):
    ...
    def apply(
        ...
        # W4A8 is decode-only. Missing or contradictory phase evidence must
        # retain the W4A16 prompt/reference path, including on MTP layer 13.
        decode_only = phase is True
        executed_mode = (
            "w4a8"
            if _uses_fruit_w4a8(
                self.quant_config.qsrt_runtime,
