...
Rank-sliced routed-expert checkpoints use B12X's planned full-rotation
...
_B12X_TRELLIS_API: Any | None = None
...
def _load_b12x_trellis() -> Any:
    """Resolve the public planned Trellis MoE API lazily."""

    global _B12X_TRELLIS_API
    if _B12X_TRELLIS_API is not None:
        return _B12X_TRELLIS_API
    try:
        from b12x.moe import trellis_moe
    except Exception as exc:
        raise RuntimeError(
            "Rank-sliced EXL3 requires b12x.moe.trellis_moe. Install "
            "a B12X build containing the planned full-rotation API."
        ) from exc
    _B12X_TRELLIS_API = trellis_moe
    return trellis_moe
