...
_GLM5_MIXED_TRELLIS_PREFILL_BLOCK_SIZE = 32
_GLM5_MIXED_TRELLIS_BLOCK32_SIGNATURES = frozenset(
    {
        ((3, 192), (4, 64)),
        ((3, 206), (4, 50)),
        ((3, 148), (4, 108)),
...
def _resolve_mixed_trellis_prefill_block_m(
    *,
    ...
    """Select the qualified GLM-5-family mixed-K prefill route block.
    ...
    partitions qualified end-to-end on GLM-5.2/5.3 checkpoints. Explicit operator
    ...
    qualified = (
        ...
        and tier_signature in _GLM5_MIXED_TRELLIS_BLOCK32_SIGNATURES
        and int(topk) == 8
        and tuple(int(value) for value in prefill_tile_config) == (128, 128, 32, 512)
    )
    if qualified:
        return _GLM5_MIXED_TRELLIS_PREFILL_BLOCK_SIZE
...
def _is_draft_layer(layer: Any) -> bool:
    ...
    GLM-5-family MTP head is an extra decoder layer named exactly like a
...
def _load_b12x_mixed_trellis() -> Any:
    ...
    api = SimpleNamespace(
        ...
        bind_mixed_trellis=module.bind_mixed_trellis,
        ...
        run_bound_mixed_trellis=module.run_bound_mixed_trellis,
    ...
    def _load_rank_sliced_bitrates(
        ...
        *,
        ...
        for layer_index in range(first, last + 1):
            ...
            raw = entry.get(field)
            # The GLM-5-family MTP overlay records routed experts under tail_tr3
...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        if rank_sliced:
            ...
            # index-based detection can resolve the role there -- a GLM-5-family
    ...
    def _mixed_trellis_tile_config(hidden_size: int, intermediate_size: int):
        ...
        # 512-wide FC2 tile removes the second persistent wave on GLM-5 models.
    ...
    def get_b12x_warmup_unit(
        ...
        if not getattr(layer, "exl3_mixed_bitrate", False):
            raise RuntimeError("B12X route-pack warmup requires mixed-bitrate EXL3")

        def compile() -> None:
            _warmup_exl3_mixed_trellis_route_pack(layer)

        return B12xWarmupUnit(
            name="EXL3 mixed-Trellis route pack",
            key=(type(self), id(layer)),
            compile=compile,
        )
...
def _warmup_exl3_mixed_trellis_route_pack(layer: RoutedExperts) -> int:
    """Materialize one mixed-Trellis layer's route-pack modules before KV sizing.
    ...
    if runtime is None:
        raise RuntimeError(
            ...
            f"{layer.layer_name}; the eager profile pass must plan the runtime "
