...
__device__ __forceinline__ int32_t find_first_position(
    int32_t key,
    const int32_t* hash_keys,
    const int32_t* hash_first_positions,
...
__global__ void build_dense_union_remap_kernel(
    ...
    int32_t* __restrict__ union_counts,
    int32_t* __restrict__ hash_keys,
    int32_t* __restrict__ hash_first_positions,
    int32_t* __restrict__ first_to_dense,
    int destinations,
    int num_requests,
    int max_requests,
  ...
  for (int64_t i = threadIdx.x; i < input_entries; i += blockDim.x) {
    ...
    for (int64_t probe = 0; probe < hash_capacity; ++probe) {
      ...
      if (previous == -1 || previous == key) {
        atomicMin(
            hash_first_positions + hash_base + slot,
  ...
  for (int64_t tile = 0; tile < input_entries; tile += blockDim.x) {
    ...
    if (threadIdx.x == 0) {
      ...
      const int32_t valid = static_cast<int32_t>(
          remaining < static_cast<int64_t>(blockDim.x)
              ? remaining
  ...
  for (int64_t i = threadIdx.x; i < input_entries; i += blockDim.x) {
    ...
    const int32_t first = find_first_position(
        key,
        hash_keys + hash_base,
        hash_first_positions + hash_base,
        hash_capacity);
...
void validate_cuda_int32_contiguous(const torch::Tensor& value, const char* name) {
...
void build_dense_union_remap(
    torch::Tensor indices,
    torch::Tensor union_indices,
    torch::Tensor remap,
    torch::Tensor union_counts,
  ...
  TORCH_CHECK(indices.dim() == 4, "indices must be [destination, request, row, topk]");
  TORCH_CHECK(num_requests > 0 && num_requests == indices.size(1), "active request count must match indices");
  TORCH_CHECK(
      union_indices.dim() == 3,
  ...
  TORCH_CHECK(union_indices.device() == device, "union_indices device mismatch");
  ...
  TORCH_CHECK(hash_first_positions.device() == device, "hash_first_positions device mismatch");
  TORCH_CHECK(first_to_dense.device() == device, "first_to_dense device mismatch");
  ...
  TORCH_CHECK((hash_capacity & (hash_capacity - 1)) == 0, "hash capacity must be a power of two");
  TORCH_CHECK(hash_capacity >= 2 * input_entries, "hash capacity must be at least 2x input entries");
  TORCH_CHECK(union_indices.size(0) == destinations, "union destination mismatch");
  TORCH_CHECK(num_requests <= max_requests, "active requests exceed union state capacity");
  TORCH_CHECK(
      remap.dim() == 4 && remap.size(0) == destinations &&
          remap.size(1) == max_requests && remap.size(2) >= indices.size(2) &&
          remap.size(3) == indices.size(3) &&
          remap.size(2) * remap.size(3) == capacity,
      "remap shape mismatch");
  TORCH_CHECK(
      union_counts.dim() == 2 && union_counts.size(0) == destinations &&
          union_counts.size(1) == max_requests,
      "count shape mismatch");
  TORCH_CHECK(hash_keys.sizes() == hash_first_positions.sizes(), "hash shape mismatch");
  TORCH_CHECK(
      hash_keys.size(0) == destinations && hash_keys.size(1) == max_requests,
      "hash state shape mismatch");
  TORCH_CHECK(first_to_dense.sizes() == union_indices.sizes(), "dense-slot shape mismatch");
  ...
  build_dense_union_remap_kernel<<<blocks, threads, 0, stream>>>(
      ...
      union_counts.data_ptr<int32_t>(),
      hash_keys.data_ptr<int32_t>(),
      hash_first_positions.data_ptr<int32_t>(),
      first_to_dense.data_ptr<int32_t>(),
      destinations,
