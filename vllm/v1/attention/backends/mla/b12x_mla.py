...
def _pack_k3_query_kernel(
    ...
    rope_out = (out_base + D_NOPE + (D_NOPE // TILE) * 4).to(
...
def pack_k3_query(q: torch.Tensor, out: torch.Tensor) -> torch.Tensor:
    ...
    if (
        ...
        or int(q.shape[-1]) != _K3_ABSORBED_HEAD_DIM
    ):
        raise TypeError("pack_k3_query expects a (rows, heads, 576) bf16 query")
    if q.stride(-1) != 1:
        raise ValueError(
            "pack_k3_query expects a query with a contiguous head dimension"
        )
    if (
        out.ndim != 3
        or out.dtype != torch.uint8
        or tuple(out.shape)
        != (int(q.shape[0]), int(q.shape[1]), _PACKED_QUERY_RECORD_BYTES)
        or out.stride(-1) != 1
        or out.stride(1) % 4
        or out.stride(0) % 4
    ):
    ...
    def forward_mqa(
        ...
        if self._uses_packed_ds_mla:
            if self._packed_query:
                if (
                    q.dtype != torch.uint8
                    or int(q.shape[-1]) != _PACKED_QUERY_RECORD_BYTES
                ):
                    raise TypeError(
