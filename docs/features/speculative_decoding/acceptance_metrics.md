...
| `draft_acceptance_rate` | Fraction of target-verified draft tokens accepted: `num_accepted_draft_tokens / num_draft_tokens`. |
...
| `num_draft_tokens` | Total target-verified draft tokens, after structured-output invalidation and adaptive verification selection. For non-adaptive methods, every valid proposal is verified. |
...
| `per_step_drafted` | Target-verified draft count at each step. Records the effective verification depth, so adaptive verification is represented without a schema change. |
...
Per-position acceptance uses position-specific verified-token denominators:

```promql
rate(vllm:spec_decode_num_accepted_tokens_per_pos_total[5m])
/
rate(vllm:spec_decode_num_draft_tokens_per_pos_total[5m])
```

A position with a zero verified-token denominator is unavailable, rather than
an observed zero acceptance rate.
