...
from vllm.models.kimi_k3.nvidia.model import KimiMLP, KimiRoutedOutputTransform
...
def _make_row_projection(
    *,
    ...
    projection.input_size = input_size
    projection.input_size_per_partition = input_size // tp_size
    ...
    projection.weight = nn.Parameter(
        torch.arange(output_size * (input_size // tp_size))
        .view(output_size, input_size // tp_size)
        .float(),
        requires_grad=False,
    )
    return projection
...
def test_routed_output_transform_reuses_consumed_prefill_input() -> None:
    projection = _make_row_projection()
    transform = KimiRoutedOutputTransform(None, projection)
...
def test_routed_output_transform_rejects_wrong_latent_width() -> None:
    projection = _make_row_projection()
    transform = KimiRoutedOutputTransform(None, projection)
    hidden_states = torch.empty(1024, 5)
    output = torch.empty(1024, 4)

    assert not transform.can_write_output(hidden_states, output)


def test_routed_output_transform_accumulates_in_bounded_tiles() -> None:
    projection = _make_row_projection(output_size=2048)
    projection.weight.data.div_(1024)
    transform = KimiRoutedOutputTransform(None, projection)
    ...
    assert actual.data_ptr() == residual_pointer
    torch.testing.assert_close(actual, expected)


def test_routed_output_transform_normalizes_consumed_latent_in_place(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    norm = _make_rms_norm(4)
    transform = KimiRoutedOutputTransform(norm, nn.Identity())
