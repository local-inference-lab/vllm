...
"""Persisting-L2 set-aside sizing for the Kimi-K3 L2 weight prefetcher."""

import pytest
import torch

from vllm.models.kimi_k3.nvidia import l2_prefetch as l2pf
...
def test_default_request_leaves_the_driver_limit_unchanged(monkeypatch):
    monkeypatch.delenv("VLLM_K3_L2_PREFETCH_PERSIST_MB", raising=False)
...
def test_invalid_numeric_environment_values_use_defaults(monkeypatch):
    monkeypatch.setenv("VLLM_K3_L2_PREFETCH_MAX_TOKENS", "invalid")
    monkeypatch.setenv("VLLM_K3_L2_PREFETCH_BUDGET_A_MB", "invalid")
    assert l2pf._int_env("VLLM_K3_L2_PREFETCH_MAX_TOKENS", 256) == 256
    assert l2pf._mb("VLLM_K3_L2_PREFETCH_BUDGET_A_MB", "20") == 20_000_000
