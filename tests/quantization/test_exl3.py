...
def test_rank_three_per_expert_trellis_is_not_loaded_as_fused_weight():
    experts = SimpleNamespace(
        layer_name="model.layers.0.mlp.experts",
        is_fused_checkpoint_transposed=False,
    )
    experts.get_expert_mapping = lambda **_: RoutedExperts.build_expert_params_mapping(
        "gate_proj",
        "down_proj",
        "up_proj",
        num_experts=1,
    ...
    experts.w13_trellis = SimpleNamespace(weight_loader=weight_loader)
    trellis = torch.zeros((8, 16, 48), dtype=torch.int16)

    loaded = list(
        RoutedExperts.load_weights(
            experts,
            [("0.gate_proj.trellis", trellis)],
        )
    )

    assert loaded == ["w13_trellis"]
    assert len(loaded_calls) == 1
    assert loaded_calls[0]["expert_id"] == 0
    assert loaded_calls[0]["shard_id"] == "w1"
    loaded_trellis = loaded_calls[0]["loaded_weight"]
    assert loaded_trellis.shape == trellis.shape
    assert loaded_trellis.data_ptr() == trellis.data_ptr()
...
def test_rank_sliced_weights_use_unified_fused_moe_contract(monkeypatch):
    ...
    class FakeFusedMoe:
        ...
        def prepare_rank_sliced_trellis_weights(self, **kwargs):
    ...
    assert api.prepare_kwargs["w13"] is slabs["w13_trellis"]
    assert api.prepare_kwargs["w2"] is slabs["w2_trellis"]
...
def test_rank_sliced_weights_pass_shared_h_rows_without_expansion(monkeypatch):
    ...
    class FakeFusedMoe:
        ...
        @staticmethod
        def prepare_rank_sliced_trellis_weights(**kwargs):
...
def test_mixed_trellis_dispatches_decode_and_one_grid_prefill(monkeypatch):
    class FakeMixedApi:
        ...
        def run_bound_mixed_trellis(self, x, weights, ids, binding, buffers):
            del buffers
            self.calls.append((int(x.shape[0]), binding.tiers, weights, ids))
            return torch.full_like(x, binding.value)
    ...
    runtime = {
        ...
        "decode": {
            ...
            "binding": SimpleNamespace(value=1, tiers=decode_tiers),
            "buffers": object(),
        },
        "prefill": {
            "launch": SimpleNamespace(value=3),
            "binding": SimpleNamespace(value=3, tiers=prefill_tiers),
            "buffers": object(),
        },
        "max_decode_m": 8,
    ...
    assert mixed_api.calls[0][1] == decode_tiers
    assert all(call[1] == prefill_tiers for call in mixed_api.calls[1:])
...
def test_rank_sliced_window_defaults_to_min_capturable_m(monkeypatch) -> None:
    ...
    # The GLM-5-family MTP head is named exactly like a target layer, so the role
...
def test_draft_role_stamp_wins_over_name() -> None:
    ...
    # GLM-5-family MTP head: named like a target, stamped draft.
    assert exl3_mod._is_draft_layer(
