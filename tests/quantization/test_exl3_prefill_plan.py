...
class _FakeMixedTrellisApi:
    ...
    def bind_mixed_trellis(self, tier0, tier1, *args):
        launch = args[-1]
        binding = SimpleNamespace(tiers=(tier0, tier1), launch=launch)
...
def test_mixed_prefill_capacity_slices_rows_and_routing():
    with _Harness(env={"VLLM_EXL3_PREFILL_CAPACITY": "128"}) as h:
        ...
        assert [binding.tiers for binding in h.mixed_api.bound] == [
            layer.exl3_mixed_trellis["tiers"],
            layer.exl3_mixed_trellis["prefill_tiers"],
        ]
