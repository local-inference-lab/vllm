...
def test_mtp_draft_graph_identity_includes_prompt_phase():
    prompt = BatchDescriptor(
        num_tokens=16,
        speculative_draft_decode_only=False,
    )
    decode = BatchDescriptor(
        num_tokens=16,
        speculative_draft_decode_only=True,
    )

    assert prompt != decode
    assert len({prompt, decode}) == 2
