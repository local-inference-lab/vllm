...
def test_caller_owned_convolution_preserves_output_and_history(
    rows, speculative_slots, capture
):
    ...
    nums_dict, batch_ptr, token_chunk_offset_ptr = compute_causal_conv1d_metadata(
        starts.cpu(), device=starts.device
    )
    kwargs = dict(
    ...
    if capture:
        ...
        for _ in range(3):
            source.normal_()
            original.copy_(source)
            history.normal_()
            expected_history.copy_(history)
            expected = causal_conv1d_fn(value, weight, None, expected_history, **kwargs)
            graph.replay()
            torch.testing.assert_close(actual, expected, rtol=0, atol=0)
            torch.testing.assert_close(history, expected_history, rtol=0, atol=0)
